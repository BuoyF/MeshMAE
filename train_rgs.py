...
from model.dataset import BodyDataset
...
def train(net, optim, scheduler, names, criterion, train_dataset, epoch, args):
    print("train executed")
    net.train()
    running_loss = 0
    running_corrects = 0
    n_samples = 0

    for it, (feats_patch, center_patch, coordinate_patch, face_patch, np_Fs, labels, mesh_paths, demographic) in enumerate(
            train_dataset):
        optim.zero_grad()
...
def test(net, names, criterion, test_dataset, epoch, args):
    ...
    for i, (feats_patch, center_patch, coordinate_patch, face_patch, np_Fs, labels, mesh_paths, demographics) in enumerate(
            test_dataset):
        # TODO: demographics to be used
